...
function piorZona(semaforo: SemaforoData) {
  const zonas = [
    semaforo.total.zona,
...
export function DashboardHero({ diasParaProva, semaforo }: DashboardHeroProps) {
  const zonaGeral = piorZona(semaforo);

  return (
    <section className="relative overflow-hidden rounded-2xl border border-border bg-asfalto">
      {/* Faixa de pedestre sutil */}
      <div
      ...
      <div className="relative flex flex-col gap-6 p-5 md:flex-row md:items-center md:justify-between md:p-8">
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            <p className="mt-1 max-w-md text-sm text-muted-foreground">
              CTB, CONTRAN e disciplinas do edital — com semáforo de
              eliminação em tempo real.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 md:gap-8">
          <div className="flex flex-col items-start gap-1 md:items-end">
            ...
            <p className="text-xs text-muted-foreground">{formatarDataProva()}</p>
