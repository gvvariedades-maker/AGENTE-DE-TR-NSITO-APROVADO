...
description: Atua como examinador sênior da banca IDECAN para o concurso Agente de Trânsito STTP Campina Grande/PB. Gera questões inéditas no DNA IDECAN, comentários Professor Elite, questões irmãs para estudo reverso e lotes JSON para seed. Use ao criar questões, comentar gabaritos, extrair padrões de provas IDECAN, popular content/questoes/, validar material ou calibrar simulados espelho 60Q.
---
...
| Edital | `conteúdo/EDITAL Nº 042026...` |
| Retificação | `conteúdo/Retificação do conteúdo programático...` |
| Lei seca trânsito | `conteúdo/RESOLUÇÃO CONTRAN Nº...` + CTB |
...
- [ ] 8. Validar JSON (schema + script)
- [ ] 9. Gerar 3 questões irmãs (mesmo microtópico, cenários diferentes)
```
...
3. Rodar: `npx tsx .cursor/skills/examinador-idecan/scripts/validate-questao.ts content/questoes/...`
4. Importar: `npm run db:seed`
...
```json
{
  ...
  "comentario": {
    "o_que_testa": "...",
    "fundamento_legal": "CTB, art. 165, §2º...",
    "passo_a_passo": ["1...", "2...", "3..."],
    "pegadinha": "...",
    "macete": "...",
...
```bash
npx tsx .cursor/skills/examinador-idecan/scripts/validate-questao.ts caminho/arquivo.json
```
...
- Usar 5 alternativas sem confirmar no edital retificado
